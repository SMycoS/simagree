...
class SearchForm(forms.Form):
  taxon = forms.IntegerField(label='Taxon', required=False,
                             widget=forms.NumberInput(attrs={'class': 'form-control'}))
  fiche = forms.IntegerField(label='Fiche', required=False,
                             widget=forms.NumberInput(attrs={'class': 'form-control'}))
  nomUsuel = forms.CharField(label='Nom', required=False, widget=forms.TextInput(
      attrs={'class': 'form-control'}))
  genre = forms.CharField(label='Genre', required=False,
                          widget=forms.TextInput(attrs={'class': 'form-control'}))
  espece = forms.CharField(label='Espèce', required=False,
                           widget=forms.TextInput(attrs={'class': 'form-control'}))
  opts = [('all', 'Non renseigné'), ('C', 'Comestible'),
          ('NC', 'Non Comestible'), ('T', 'Toxique'), ('MO', 'Mortel')]
  comestible = forms.ChoiceField(label='Comestible', widget=forms.Select(
      attrs={'class': 'form-control'}), choices=opts)
  presentSms = forms.BooleanField(label='Afficher uniquement les espèces présentes à la SMS',
                                  widget=forms.CheckboxInput(attrs={'class': 'form-control'}), required=False)
  displaySyno = forms.BooleanField(label='Afficher les synonymes', widget=forms.CheckboxInput(
      attrs={'class': 'form-control'}), required=False)
  a_imprimer = forms.BooleanField(label='A imprimer', widget=forms.CheckboxInput(
      attrs={'class': 'form-control'}), required=False)
  ...
  def clean_comestible(self):
    ...
    if form_sms and (form_com == ""):
      raise forms.ValidationError(
          u"La comestibilité est obligatoire si le taxon est présent à la SMS")
    return form_com
...
class AddFormPartial(forms.ModelForm):
  tax = forms.IntegerField(
      label='Taxon', widget=forms.NumberInput(attrs={'class': 'form-control'}))
...
class EditListTaxonsForm(forms.ModelForm):
  ...
  def __init__(self, *args, **kwargs):
    ...
    for k in self.init_choices:
      ...
      self.opts.append((k['taxon'], str(k['taxon']) + ' - ' + k['genre'] +
                        ' ' + k['espece'] + ' ( ' + k['variete'] + ' ' + k['forme'] + ' )'))
