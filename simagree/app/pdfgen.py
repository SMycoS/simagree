...
def generateFiche(pdf_filename, vars, size=(115*mm, 85*mm)):
  ...
  style_obs = ParagraphStyle(name='Obs', fontSize=9,
                             leading=9, fontName='Helvetica-Oblique')
  style_usuel = ParagraphStyle(
      name='SynoUsuel', fontSize=12, alignment=1, leading=12)

  # Genre - Espece
  style_nomenc = ParagraphStyle(
      name='Nomenclature', fontSize=18, alignment=1, fontName='Helvetica-Bold', leading=22)
  # Variete - Forme
  style_nomenc_bis = ParagraphStyle(
      name='NomenclatureBis', fontSize=14, alignment=1, leading=11, fontName='Helvetica-Bold')
  # Noms
  style_noms = ParagraphStyle(
      name='Noms', fontSize=12, leading=12, fontName='Helvetica-Bold')
  ...
  if 'noms' in vars.keys() and vars['noms'] != "":
    frameW = sizeX * 0.65
    frameH = marginY * 7
    frame_noms = Frame(marginX, midY - 4.5 * marginY,
                       frameW, frameH, showBoundary=0)
    noms = []
  ...
  if 'usuel_genre' in vars.keys():
    frameW = 0.66 * sizeX
    frameH = marginY * 3.5
    frame_syno = Frame(0.165 * sizeX, midY + 2 * marginY,
                       frameW, frameH, showBoundary=0)
    if vars['usuel_variete'] != "":
      ...
      varfor_str = ''
    ustr = '( = ' + vars['usuel_genre'] + ' ' + \
        vars['usuel_espece'] + varfor_str + ' )'
  ...
  if 'comestibilite' in vars.keys() and vars['comestibilite'] != "":
    ...
    cnv.drawImage(com_img, sizeX - marginX - img_width, midY -
                  (1.5 * marginY), width=img_width, height=img_width)
  ...
  # Observations
  if 'obs' in vars.keys():
    observations = [Paragraph(vars['obs'][0], style_obs),
  ...
  barcode = code39.Standard39(
      vars['taxon'], barWidth=0.5*mm, barHeight=3.5*mm, checksum=0)
...
def generateFicheTheme(pdf_filename, vars, size=(325.984, 240.945)):
  ...
  marginY = 10
  cnv = canvas.Canvas(pdf_filename, pagesize=size, verbosity=1)  # 115 x 85 mm
  style_eco = ParagraphStyle(
      name='ObsEco', fontSize=10, leading=10, fontName='Helvetica-Oblique')
  style_notes = ParagraphStyle(
      name='ObsNotes', fontSize=9, leading=9, fontName='Helvetica-Bold')
  ...
  style_nomenc = ParagraphStyle(
      name='Nomenc', fontName='Helvetica-BoldOblique', fontSize=12)
  style_nomenc_bis = ParagraphStyle(
      name='Nomenc', fontName='Helvetica-Oblique', fontSize=10)
  style_noms = ParagraphStyle(
      name='Noms', fontSize=16, alignment=1, fontName='Helvetica-Bold', leading=16)
  ...
  frameH = marginY * 2.5
  frame_titre = Frame(4.5 * marginX, sizeY - marginY * 2.7, frameW, frameH,
                      showBoundary=0, leftPadding=2, topPadding=2, rightPadding=2, bottomPadding=2)
  ...
  frameW = 0.9 * sizeX
  frameH = 3 * marginY
  frame_nomenc = Frame(marginX * 0.6, sizeY - (10 * mm) - frameH, frameW, frameH,
                       showBoundary=0, leftPadding=2, topPadding=2, rightPadding=2, bottomPadding=2)
  nomenc = []
  ...
  if 'usuel_genre' in vars.keys():
    nomenc.append(Paragraph(
        '( = ' + vars['usuel_genre'] + ' ' + vars['usuel_espece'] + ' ) ', style_nomenc_bis))
    if vars['usuel_variete'] != "":
      nomenc.append(
  ...
  if 'noms' in vars.keys() and vars['noms'] != "":
    frameW = sizeX * 0.75
    frameH = marginY * 6
    frame_noms = Frame(marginX, midY - 3 * marginY, frameW, frameH, showBoundary=0,
                       leftPadding=2, topPadding=2, rightPadding=2, bottomPadding=2)
    noms = []
    for nom in vars['noms'].splitlines():
      if nom != "":
  ...
  if 'comestibilite' in vars.keys() and vars['comestibilite'] != "":
    ...
    cnv.drawImage(com_img, sizeX - marginX - img_width, midY +
                  (0.5 * marginY), width=img_width, height=img_width)
    cnv.drawRightString(sizeX - marginX, midY - (0 * marginY) - 5, com_txt)

  # Rectangle du bas
  cnv.setLineWidth(1)
  frame_obs = Frame(0 + marginX, 5 + marginY, sizeX - (2 * marginX), sizeY / 6,
                    showBoundary=1, leftPadding=2, topPadding=2, rightPadding=2, bottomPadding=2)

  # Observations
  if 'obs' in vars.keys():
    notes = [Paragraph(vars['obs'][0], style_notes)]
    eco = [Paragraph(vars['obs'][1], style_eco)]
    frame_eco = Frame(0 + marginX, 5 + (1 * marginY) + sizeY / 6, sizeX - (2 * marginX), 2.6 *
                      marginY, showBoundary=0, leftPadding=2, topPadding=2, rightPadding=2, bottomPadding=2)
...
def bulk_pdf(fiche_list, rep):
  ...
  for cpt, fiche in enumerate(fiche_list):
    if cpt < liste_size - reste:
      ...
      for i, f in enumerate([f1, f2, f3]):
        ...
        output_pdf.mergeTranslatedPage(
            f_pdf, offset_x, 30 + i * offset_y, expand=True)
  ...
  if reste != 0:
    ...
    if reste == 1:
      ...
      output_pdf.mergeTranslatedPage(
          f_pdf, offset_x, 30 + offset_y, expand=True)
    elif reste == 2:
      fr1 = open(rep + fiche_list[liste_size - 1], 'rb')
      f_pdf = PyPDF2.PdfFileReader(fr1).getPage(0)
      output_pdf.mergeTranslatedPage(
      ...
      output_pdf.mergeTranslatedPage(
          f_pdf, offset_x, 30 + 2 * offset_y, expand=True)
